  ...
  return (
    <div className="min-h-full bg-stone-50 dark:bg-stone-950 pb-24 text-stone-900 dark:text-stone-100">
      <div className="max-w-4xl mx-auto px-2 sm:px-4 pt-3">
        <div className="bg-white dark:bg-stone-900 rounded-3xl border border-stone-200/90 shadow-xs overflow-hidden flex flex-col md:flex-row min-h-[75vh]">
          {/* Left Sidebar: Conversations List */}
          <div
            className={`w-full md:w-80 border-r border-stone-200 dark:border-stone-800 bg-white dark:bg-stone-900 flex flex-col ${
              selectedConvId && id ? 'hidden md:flex' : 'flex'
            }`}
          >
            <div className="p-4 border-b border-stone-100 dark:border-stone-800 flex items-center justify-between">
              <h1 className="text-base font-bold text-stone-900 dark:text-stone-100">Mesajlar</h1>
              <span className="text-xs text-stone-400 font-semibold">{conversations.length} sohbet</span>
            </div>

            <div className="overflow-y-auto flex-1 divide-y divide-stone-100 dark:divide-stone-800">
              {isLoadingConvs && (
          ...
          {activeConv ? (
            ...
            >
              {/* Chat Header */}
              <div className="p-3.5 bg-white dark:bg-stone-900 border-b border-stone-200/90 flex items-center justify-between">
                <div className="flex items-center gap-3">
                  <button
                    ...
                    className="md:hidden p-1.5 rounded-lg text-stone-600 dark:text-stone-400 hover:bg-stone-100"
                  >
                  ...
                  <div>
                    <div className="flex items-center gap-1.5">
                      <h3 className="text-xs font-bold text-stone-900 dark:text-stone-100">{activeConv.participant.fullName}</h3>
                  ...
                  if (msg.type === 'system_card') {
                    return (
                      <div key={msg.id} className="text-center my-2">
                        <span className="inline-block px-3 py-1 bg-stone-200/80 rounded-full text-[10px] font-semibold text-stone-600 dark:text-stone-400">
                          {msg.content}
                        </span>
                      </div>
                    );
                      ...
                      <div
                        ...
                        className={`max-w-xs sm:max-w-sm rounded-2xl p-3.5 border ${
                          isMe
                            ...
                            : 'mr-auto bg-white dark:bg-stone-900 text-stone-900 dark:text-stone-100 border-stone-200 dark:border-stone-800 shadow-xs'
                        }`}
                      >
                        ...
                        <button
                          ...
                          className="w-full py-2 bg-white dark:bg-stone-900 text-emerald-950 font-bold rounded-xl text-xs hover:bg-emerald-50 transition-colors shadow-xs"
                        >
                      ...
                      <div
                        className={`max-w-[78%] sm:max-w-md px-3.5 py-2.5 rounded-2xl text-xs leading-relaxed ${
                          isMe
                            ...
                            : 'bg-white dark:bg-stone-900 text-stone-800 dark:text-stone-200 border border-stone-200/80 rounded-tl-xs shadow-xs'
                        }`}
                      >
                        <p>{msg.content}</p>
                      </div>
              ...
              <div className="px-3 py-2 bg-white dark:bg-stone-900 border-t border-stone-100 dark:border-stone-800 flex items-center gap-1.5 overflow-x-auto no-scrollbar">
                <button
                  ...
                  className="px-2.5 py-1 rounded-full bg-stone-100 dark:bg-stone-800 hover:bg-emerald-50 text-[11px] font-semibold text-stone-700 dark:text-stone-300 hover:text-emerald-800 whitespace-nowrap transition-colors"
                >
              ...
              {/* Message Input Form */}
              <form onSubmit={handleSendMessage} className="p-3 bg-white dark:bg-stone-900 border-t border-stone-200/90 flex items-center gap-2">
                <input
                  type="text"
                  value={inputText}
                  onChange={(e) => setInputText(e.target.value)}
                  placeholder="Mesajınızı yazın..."
                  className="flex-1 px-4 py-2.5 bg-stone-50 dark:bg-stone-950 border border-stone-200 dark:border-stone-800 rounded-2xl text-xs outline-hidden focus:bg-white focus:border-emerald-700 transition-colors"
                />
                <button
                  type="submit"
